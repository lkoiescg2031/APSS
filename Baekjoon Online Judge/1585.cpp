...
int main() {
	...
	for (auto& i : in) scanf("%d", &i);
	for (auto& i : out) scanf("%d", &i);
	scanf("%d %d", &t, &f);

	MCMF mcmf(2 * n + 2, 2 * n, 2 * n + 1),
		mcmf2(2 * n + 2, 2 * n, 2 * n + 1);
	...
	for(int u=0;u<n;u++)
		for (int v = 0; v < n; v++) {
			if (out[v] <= in[u]) continue;
			int time = out[v] - in[u],cost = 0;
			if (time < t) cost = min((t - time) * (t - time), f);
			mcmf.add_edge(u, v + n, 1, cost);
			mcmf2.add_edge(u, v + n, 1, -cost);
		}
	auto ans = mcmf.flow();
	if (ans.first != n) printf("-1");
	else printf("%d %d", ans.second, -mcmf2.flow().second);
	return 0;
}