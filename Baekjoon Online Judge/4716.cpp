...
int main() {
	...
	while (true) {
		...
		mcmf.add_edge_from_source(0, a, 0);
		mcmf.add_edge_from_source(1, b, 0);

		for (int i = 0; i < n; i++) {
			int flow, acost, bcost;
			scanf("%d %d %d", &flow, &acost, &bcost);
			mcmf.add_edge_to_sink(i + 2, flow, 0);
			mcmf.add_edge(0, i + 2, flow, acost);
			mcmf.add_edge(1, i + 2, flow, bcost);
		}
		printf("%d\n", mcmf.flow().second);
	}
	return 0;
